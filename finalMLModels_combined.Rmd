...
Models:
Simple/Multiple Regression, Logistic Regression, Binomial Regression, k-Nearest Neighbors (Regression/Classification), Naïve Bayes, SVM, Spline Regression, Neural Networks
...
# We will stick with the XGB Model for the final visual prediction as it is tuned 
# better, but you can easily uncomment the rf_model prediction if your RF performed
# better
xgb_test_25 <- model.matrix(~ . - 1, data = cbb25_test[, features_RF])

probs_2025_xgb <- predict(xgb_model, newdata = xgb_test_25)
pred_2025_xgb <- ifelse(probs_2025_xgb > thresholds[best_index_XGB], 1, 0)

## RANDOMFOREST PREDS
probs_2025_rf <- predict(rf_model, newdata = cbb25_test, type = "prob")[,2]
pred_2025_rf <- ifelse(probs_2025_rf > thresholds[best_index_RF], 1, 0)

for(i in seq_along(probs_2025)){
  names(probs_2025_xgb) <- cbb25_test$Team
}

for(i in seq_along(probs_2025)){
  names(probs_2025_rf) <- cbb25_test$Team
}

predictions_xgb <- data.frame(
  probs = probs_2025_xgb,
  preds = pred_2025_xgb,
...
predictedFinalFours_xgb <- predictions_xgb %>%
  filter(preds == 1) %>%
  mutate(probs = round(probs,2)) %>%
  arrange(desc(probs))

predictedFinalFours_rf <- predictions_rf %>%
  filter(preds == 1) %>%
  mutate(probs = round(probs,2)) %>%
  arrange(desc(probs))
